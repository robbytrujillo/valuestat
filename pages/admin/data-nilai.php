...
// Hitung total data
$result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM nilai_siswa");
$row_total = mysqli_fetch_assoc($result);
$total = $row_total['total'];
$pages = ceil($total / $limit);

// Ambil data sesuai halaman
$nilai_siswa = mysqli_query($conn, "SELECT * FROM nilai_siswa ORDER BY id DESC LIMIT $start, $limit");
$no = $start + 1;
      ...
      <div class="card p-4 shadow-md" style="border-radius: 5%;">
        ...
        <form action="" method="POST" class="mb-4">
          <div class="form-row">
            ...
            <div class="col-md-2">
              <!-- <button type="submit" name="tambah" class="btn btn-success btn-md btn-block rounded-pill">Tambah</button> -->
               <a href="input-nilai.php" class="btn btn-success btn-md rounded-pill">Tambah</a>
            </div>
          </div>
        </form>
        ...
        <table class="table table-bordered-0 table-striped">
          ...
          <tbody>
            <?php while ($row = mysqli_fetch_assoc($nilai_siswa)) : ?>
            <?php
